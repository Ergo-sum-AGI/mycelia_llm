# ============================================
# MYCELIA LM Architecture (v8.4.1) — Norm Cap Governor
# Merged: User's vectorized clamp + GPU-native telemetry + v8.4.1 structure
# ============================================
...
class MyceliaConfig:
    ...
    norm_cap: float = 400.0  # v8.4.1: Global residual stream norm cap
...
class MycelialConsensus(nn.Module):
    ...
    def forward(self, head_outputs: torch.Tensor, step: int = 0, layer_idx: int = 0):
        ...
        if self.use_dynamic_threshold:
            layer_factor = 0.8 + (layer_idx / 6.0) * 0.6
        ...
        veto_factor = acclamation_mask + (1.0 - acclamation_mask) * 0.3
...
class MycelialBlock(nn.Module):
    def __init__(self, config: MyceliaConfig, layer_idx: int):
        ...
        self.n_heads = config.n_heads
        self.d_head = config.d_model // config.n_heads
        self.norm_cap = getattr(config, 'norm_cap', 200.0)
        
        # GPU-native telemetry tracking (no .item() in hot path)
        self.register_buffer('_cap_hits', torch.zeros(1, dtype=torch.long))
    ...
    def forward(
        ...
        # ─── VECTORIZED NORM CAP GOVERNOR (v8.4.1) ─────────────────────────
        # Purely vectorized, loop-free, zero CPU-GPU sync in hot path.
        # If norm <= norm_cap: cap_factor = 1.0 (no-op)
        # If norm > norm_cap: cap_factor = norm_cap / norm (rescales to cap)
        token_norms = torch.norm(x, p=2, dim=-1, keepdim=True)
        cap_factor = torch.clamp(self.norm_cap / (token_norms + 1e-6), max=1.0)
        x = x * cap_factor
        
        # GPU-native telemetry (deferred .item() to logger interval)
        with torch.no_grad():
            active_hits = (token_norms > self.norm_cap).float()
            self._cap_hits += active_hits.sum().long()
            self._total_tokens += active_hits.numel()
        
        final_round_info['cap_hit_ratio'] = float(active_hits.mean().item())
...
if __name__ == "__main__":
    ...
    print(f"MyceliaLM v8.4.1: {n:,} parameters")
    print(f"Norm cap: {config.norm_cap}")
    print("Vectorized Norm Cap + Consensus Governor active.")
    print("Checkpoint-compatible with v8.2/v8.3/v8.4 weights.")