# ============================================
# MYCELIA LM Architecture (v7.2) - currently 579 LOC
# Changes from v7.1:
...
class MycelialConsensus(nn.Module):
    ...
    def forward(self, head_outputs: torch.Tensor, step: int = 0, layer_idx: int = 0):
        ...
        # ─── STEP 3: Compute Token Variance (needed for adaptive threshold) ─
        token_variance = variance.mean(dim=-1, keepdim=True)  # (B, T, 1)

        # ─── STEP 4: Adaptive Threshold (v8.1 fix) ────────────────────────
        # CRITICAL FIX: The old fixed-threshold approach was vetoing 95-100% of
        # tokens because actual variance (0.8-5.3) >> threshold (0.03-0.40).
        # 
        # New approach: threshold scales with the ACTUAL variance distribution.
        # We compute a robust scale estimate (median absolute deviation) and set
        # threshold to a multiple of that scale. This makes consensus responsive
        # to the model's current state, not a hyperparameter guess.

        # Compute robust variance statistics (no .item() calls — all tensor ops)
        flat_var = token_variance.view(-1)  # (B*T,)
        ...
        if self.use_dynamic_threshold:
            # Base threshold = 1.5× robust scale (allows ~67% through for normal dist)
            # Layer factor: early layers more permissive, late layers stricter
            layer_factor = 0.8 + (layer_idx / 6.0) * 0.6  # 0.8 → 1.4
            threshold = 1.2 * var_scale * layer_factor  # v8.2: tighter consensus
            # Hard bounds to prevent degenerate cases
        ...
        # ─── STEP 5: Vectorized Keep Mask & Veto Attenuation ──────────────
        acclamation_mask = (token_variance < threshold).float().unsqueeze(1)

        veto_factor = acclamation_mask + (1.0 - acclamation_mask) * 0.75  # v8.2: stronger suppression
        consensus = consensus * veto_factor

        # ─── STEP 6: Telemetry (all tensor ops, no .item()) ───────────────
        # v8.1: Coherence measures what fraction of tokens are below threshold
        # (acclaimed), not a clamped ratio. This gives meaningful signal even
        # when some tokens exceed threshold.
        max_variance = token_variance.max()
        # Fraction of tokens with variance below threshold = acclamation rate
        acclamation_rate = (token_variance < threshold).float().mean()
        coherence = acclamation_rate  # Direct measure: 1.0 = all acclaimed, 0.0 = all vetoed
        ...
        # ─── STEP 7: Three-state classification (vectorized) ──────────────
