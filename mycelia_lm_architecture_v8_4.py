# ============================================
# MYCELIA LM Architecture (v8.4) — Consensus-as-Governor
# Changes from v8.3:
# 1. Removed FCA (Fibonacci Coherence Attenuation) — redundant with fixed consensus
# 2. Threshold multiplier: 1.2x MAD → 0.8x MAD (consensus itself is the governor)
# 3. Veto attenuation: 0.75x → 0.3x (veto IS suppression, not attenuation)
# 4. Alphas restored to native unbounded (model learns alignment to pass threshold)
# 5. Checkpoint-compatible: no new buffers, no state_dict changes
# Philosophy: The consensus mechanism must be the structural governor.
# If heads align, tokens pass. If heads clash, tokens are crushed.
# The model's only path to low loss is genuine early-layer alignment.
# ============================================
...
class MycelialConsensus(nn.Module):
    ...
    def forward(self, head_outputs: torch.Tensor, step: int = 0, layer_idx: int = 0):
        ...
        if self.use_dynamic_threshold:
            ...
            threshold = 0.8 * var_scale * layer_factor  # v8.4: consensus IS the governor
        ...
        veto_factor = acclamation_mask + (1.0 - acclamation_mask) * 0.3  # v8.4: veto IS suppression
...
if __name__ == "__main__":
    ...
    print(f"MyceliaLM v8.4: {n:,} parameters")
    print("Consensus-as-Governor active. Checkpoint-compatible with v8.2/v8.3 weights.")