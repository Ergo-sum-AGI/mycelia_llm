...
# v8.4 (dubbled layers)- Bulletproof resumption, structured telemetry, zero memory leaks
...
# Temporary: Resume from 12-layer seed
# BEST_CKPT = "/home/ec2-user/SageMaker/mycelia_checkpoints/mycelia_12layers_seed.pt"
LATEST_CKPT = os.path.join(CKPT_DIR, "mycelia_latest.pt")
# BEST_CKPT = os.path.join(CKPT_DIR, "mycelia_best.pt")
BEST_CKPT = os.path.join(CKPT_DIR, "mycelia_latest.pt") # shortcut to latest
...
print("\n" + "="*70)
print("🍄 MYCELIA FINAL TRAINING v8.4")
...
for step in tqdm(range(step, step + 250000), desc="Training", initial=step):
    ...
    # ─── EMERGENCY CHECKPOINT (every LOG_EVERY steps) ────────────────
    # v8.2: Always save LATEST at log intervals, regardless of whether it's BEST
    # This ensures we never lose more than 1000 steps of progress
...
# ─── LOGGING (EVERY 1000 STEPS) ──────────────────────────────────────────
    if step % LOG_EVERY == 0 and step > 0:
        ...
        if hasattr(model, '_last_info') and model._last_info:
            ...
            # ─── NORM CAP TELEMETRY ──────────────────────────────────────
            cap_hit_ratio = info.get('cap_hit_ratio', 0.0)
        ...
        if cap_hit_ratio > 0 or max_raw_norm > 0:
            print(f"   Cap: hit={cap_hit_ratio*100:.1f}% "
                  f"max_norm={max_raw_norm:.1f} "
                  f"mean_norm={mean_raw_norm:.1f}")
        # ──────────────────────────────────────────────────────────────────
        
        sys.stdout.flush()

    # Best checkpoint — SAVE IMMEDIATELY, don't wait for SAVE_EVERY
    if avg_loss < best_loss:
        ...
        # v8.2: Always attempt save, with explicit error reporting
        try:
            ...
            if 'model_state_dict' in verify:
                ...
                print(f"\n⚠️  BEST save verification failed at step {step}")
        except Exception as e:
            print(f"\n🚨 BEST save FAILED at step {step}: {e}")
        sys.stdout.flush()

    # Regular checkpoint (every SAVE_EVERY steps)
...
# v8.2: Always save both LATEST and BEST at end, with explicit error handling
