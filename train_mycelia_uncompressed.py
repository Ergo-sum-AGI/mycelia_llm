...
BATCH_SIZE = 2
ACCUM_STEPS = 8
LR = 2.5e-4  # ← CHANGE THIS (was 5e-5)
WEIGHT_DECAY = 0.01
...
# ─── CUSTOM LR SCHEDULER WITH WARMUP + COSINE DECAY ──────────────────────

class MyceliaLRScheduler:
    """
    Linear warmup (100 steps) followed by cosine decay.
    Peak LR can be bumped to 2e-4 or 3e-4 to break language bias.
    """
    
    def __init__(self, optimizer, total_steps=610350, warmup_steps=100, 
    ...
    def get_warmup_status(self):
        if self.current_step <= self.warmup_steps:
            return f"Warmup {self.current_step}/{self.warmup_steps}"
        else:
            return "Annealing"

# Initialize the scheduler
total_steps_for_5B = 5_000_000_000 // (BATCH_SIZE * ACCUM_STEPS * MAX_SEQ_LEN)  # ~610,350 steps

lr_scheduler = MyceliaLRScheduler(
    optimizer=opt,
    total_steps=total_steps_for_5B,  # ~610K steps for 5B tokens
    warmup_steps=100,
    peak_lr=2.5e-4,    # ← BUMPED from 5e-5 to 2.5e-4
    min_lr=1e-5
)
...
# Create scheduler (will be reinitialized on resume if needed)
lr_scheduler = MyceliaLRScheduler(
    optimizer=opt,
...
if os.path.exists(LATEST_CKPT):
    ...
    # ─── RESET WARMUP BY REINITIALIZING SCHEDULER ──────────────────────────
    print(f"\n🔥 Reinitializing scheduler for resumed training...")
    lr_scheduler = MyceliaLRScheduler(
        optimizer=opt,
        total_steps=total_steps_for_5B,
        warmup_steps=100,
    ...
    # Set current step to global_step so warmup starts from here
...
for step in tqdm(range(MAX_STEPS), desc=f"Training", initial=global_step):
    ...
    print(f"🔍 _last_info keys: {list(model._last_info.keys())}")
    print(f"   Coherence: {model._last_info.get('coherence', 'NOT FOUND')}")
...
if hasattr(model, '_last_info'):
    print(f"🔍 Coherence: {model._last_info.get('coherence', 'NOT FOUND')}")
    print(f"   Variance: {model._last_info.get('variance', 'NOT FOUND')}")
    print(f"   Threshold: {model._last_info.get('threshold', 'NOT FOUND')}")
    ...
    if (step + 1) % ACCUM_STEPS == 0:
        ...
        current_lr = lr_scheduler.step()  # ← This handles warmup + cosine decay
